...
HWND C_AnimationWindow::Create (HWND hwndParent, HMENU id, C_ImageSet * is,
								DWORD dwStyle, TCHAR* szClassName)
{
	...
	if (pAnimation->nImages>0)
		m_hWnd=CreateWindow (szClassName,L"",dwStyle,
			0,0,0,0,
			hwndParent,(HMENU)id,
...
// ****************************************************************************
int C_Image::LoadBMP (TCHAR* szFileName)
{
	int n;
	...
	ifstream bmpfile (_wfopen(szFileName,L"rb"));
...
int C_ImageSet::LoadGIF (char * szFileName)
{
	...
	ifstream giffile (szFileName,ios::in|ios::binary);
